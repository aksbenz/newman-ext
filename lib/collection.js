...
function split_v1(collection, count, isParallel = false) {
...
function filter_v1(folder, parent, includeFolders, l = '|__') {
    ...
    if (!includeFolders.includes(folder.name)) {
        ...
        if (!isLeaf_v1(folder)) {
            for (var i = folder.items.count() - 1; i >= 0; i--) {
                ...
                if (item.items)
                    filter_v1(item, folder, includeFolders, l + '|__');
...
// If description is empty or matches the tag expression then include the request, else remove the request
function tagFilter(folder, parent, tags, excludeEmpty, l = '|__') {
    ...
    if (folder.item) {
        for (var i = folder.item.length - 1; i >= 0; i--) {
            ...
            // If a folder then recurse
            if (item.item)
                tagFilter(item, folder, tags, excludeEmpty, l + '|__');
            // If a request then remove it
            else {
                ...
                if ((_.isNil(_.get(item, 'request.description')) && excludeEmpty) || (!_.isNil(_.get(item, 'request.description')) && !tags.evaluate(words))) {
...
function filter(folder, parent, includeFolders, l = '|__') {
    ...
    if (!includeFolders.includes(folderName)) {
        // Check if it has any sub-folders
        if (!isLeaf(folder)) {
            // Important to traverse array backwards from end, since we will be removing elements from array
            for (var i = folder.item.length - 1; i >= 0; i--) {
                let item = folder.item[i];
...
function exclude(folder, parent, excludeFolders, l = '|__') {
    let folderName = folder.name || folder.info.name;
    folder.id = uuid();
    // console.log('EXCLUDE FOLDERS: ' + folderName);
    log('DEBUG', l + 'START:' + folderName);
    for (var i = folder.item.length - 1; i >= 0; i--) {
        let item = folder.item[i];
        let itemName = _.get(item, 'name');
        log('DEBUG', l + 'ITEM:' + itemName);
        if (item.item) {
            if (excludeFolders.includes(itemName)) {
                log('DEBUG', l + 'DELETE:' + itemName);
                _.pullAt(folder.item, i);
            } else
                exclude(item, folder, excludeFolders, l + '|__');
        }
    }
    log('DEBUG', l + 'END:' + folderName);
    _.unset(folder, 'id');
